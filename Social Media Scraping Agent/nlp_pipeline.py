...
def llm_call(prompt: str, system: str, max_tokens: int = 500) -> str:
    """Single call to the local Ollama model with basic retry on failure."""
    for attempt in range(3):
        try:
            ...
            print(f"    [!] Ollama error (attempt {attempt+1}/3): {str(e)[:120]}")
    return ""
...
def translate(arabic_text: str) -> str:
    ...
    result = llm_call(arabic_text, system, max_tokens=300)
...
def classify(english_text: str) -> dict:
    ...
    raw = llm_call(english_text, system, max_tokens=200)
